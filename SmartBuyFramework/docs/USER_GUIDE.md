...
HC 支持列表、快捷、批量购买，以及以下只读/合成功能：

```bash
# 查询 NEWBEE 门票等藏品最近最多 50 笔成交；不创建订单
node cli.js "hc成交-手机号-登录密码-NEWBEE门票"

# 按合成活动名称自动同步配方、查询账号素材实例并合成
node cli.js "hc合成-手机号-登录密码-合成名称"
```

`hc成交` 会返回成交编号、价格和成交时间。`hc合成` 在任一素材不足时会停止，
不会提交合成请求；活动配方目录保存在 `config/combinations/hc.js`，其中不保存
账号、密码或素材实例 ID。
